...
const PR = () => {
  const [isVisibleMediaViewerForGhit, setIsVisibleMediaViewerForGhit] = useState(false);
  const [selectedIndexForGhit, setSelectedIndexForGhit] = useState(0);
  const [isVisibleMediaViewerFor311, setIsVisibleMediaViewerFor311] = useState(false);
  const [selectedIndexFor311, setSelectedIndexFor311] = useState(0);
  const [isVisibleMediaViewerForSatNav, setIsVisibleMediaViewerForSatNav] = useState(false);
  const [selectedIndexForSatNav, setSelectedIndexForSatNav] = useState(0);
  const [isVisibleMediaViewerForBonenkai, setIsVisibleMediaViewerForBonenkai] = useState(false);
  const [selectedIndexForBonenkai, setSelectedIndexForBonenkai] = useState(0);
  const [isVisibleMediaViewerForYoshimotozaka, setIsVisibleMediaViewerForYoshimotozaka] = useState(false);
  const [selectedIndexForYoshimotozaka, setSelectedIndexForYoshimotozaka] = useState(0);
    ...
    <div className="PR">
      <div className="PR__contents">
        ...
        <section className="PR__contentsGhit">
          <Heading level={2} text="GHIT Fund | 世界の人々を感染症から救う - Yahoo! JAPAN" />
          <ul className="PR__contentsGhitTags util-BadgeList">
            <li className="util-BadgeList__item">
              <Badge color="orange" text="クライアントワーク" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="全工程担当" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="社会貢献" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="ノーベル賞" />
            </li>
          </ul>

          <p>
            GHIT Fund 社からの依頼で、グローバルヘルスを啓蒙するウェブサイトを作ることになり、制作を担当しました。
            <br />
            グローバルヘルスの世界では大物である博士たちにインタビューし、撮影し、記事をまとめ...
            コンテンツづくりから社内のチームでわいわい手掛けるという、とても楽しい現場となりました（ライターは編集部、撮影はカメラ部）。
            <br />
            <span className="util-Text--highlighted">
              クライアントのヒアリングや撮影ディレクションなど、デザインやコーディング以外でもウェブサイト制作に関する部分はほぼ一人で担当
            </span>
            しました。
          </p>
          <p>
            サイトの公開後に、インタビューした博士の一人が
          ...
          <div className="PR__contentsGhitImage">
            <button onClick={() => showMediaViewerForGhit(0)} className="PR__contentsGhitImageThumbnail">
              <img src={thumbnailGhit} alt="GHIT Fund グローバルヘルス特集の画像" width="100%" height="auto" />
            </button>
        ...
        <section className="PR__contents311">
          ...
          <ul className="PR__contents311Tags util-BadgeList">
            <li className="util-BadgeList__item">
              <Badge color="orange" text="社会貢献" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="毎年恒例企画" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="3.11" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="エモーショナル" />
            </li>
          </ul>
          ...
          <p>
            ...
            <br />
            また3月11日に「3.11」というキーワードの検索数に応じた金額を東北に寄付するという内容も盛り込みました。
          </p>
          <p>
            結果、
            <span className="util-Text--highlighted">予想（＝用意していた予算）を遥かに上回る検索数を叩き出し、</span>
            上層部に相談の末、予算を超えた検索数通りの金額を募金をすることができました。
          </p>
          <p>
            企画立案から3月11日までの時間が短く（アイデアが出たのが割と直前だったため）、ドタバタした状況の中なんとかサイトの公開を3月11日に間に合わせました。
            <br />
            当日「3.11」の検索に参加した一般ユーザーの方から「英語版もほしい」という声をいただき、数日後に言語切り替え機能も追加で実装しました。
          </p>
          <p>
            現在に至るまで未だ毎年続けられている企画の初年度と2年目を担当しましたが、
            <span className="util-Text--highlighted">初年度のエモーショナルな展開</span>
            は後にも先にも経験しがたいものでした。
          </p>
          <div className="PR__contents311Image">
            <button onClick={() => showMediaViewerFor311(0)} className="PR__contents311ImageThumbnail">
              <img src={thumbnail311} alt="3.11企画の画像" width="100%" height="auto" />
            </button>
        ...
        <section className="PR__contentsSatNav">
          ...
          <ul className="PR__contentsSatNavTags util-BadgeList">
            <li className="util-BadgeList__item">
              <Badge color="orange" text="パララックス" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="GIFアニメーション" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="横スクロール" />
            </li>
          </ul>
          ...
          <p>
            SP版は同じコンセプトを踏襲しつつも、横スクロールが難しいので、車のアニメーションや背景は専用のものを用意し、縦スクロールで同じ世界観を表現できるようにしました。
          </p>
          <div className="PR__contentsSatNavImage">
            <button onClick={() => showMediaViewerForSatNav(0)} className="PR__contentsSatNavImageThumbnail">
              <img src={thumbnailSatNav} alt="Yahoo!カーナビ 秋の3大キャンペーンの画像" width="100%" height="auto" />
            </button>
        ...
        <section className="PR__contentsBonenkai">
          <Heading level={2} text="幹事5,000人に忘年会大調査 - Yahoo!予約飲食" />
          <ul className="PR__contentsBonenkaiTags util-BadgeList">
            <li className="util-BadgeList__item">
              <Badge color="orange" text="インフォグラフィックス" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="イラストレーション" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="外注分業" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="楽しい現場" />
            </li>
          </ul>
          ...
          <p>
            ...
            <br />
            出したアイデアが
            <span className="util-Text--highlighted">大量のピクトグラムやイラスト画像</span>
            を必要としたため、制作期間の兼ね合いでイラスト部分を外注することに。
          </p>
          <p>
            イラストはすべて自ら手描きで描いて指示コメントとともに発注したところ、思ったより担当してくれるスタッフの人数が多く、フィードバックやクオリティのばらつきの調整に自分の時間を割くこととなりました。
            <br />
            しかしながら自分のアイデアが比較的ふざけていたため、
            <span className="util-Text--highlighted">外注スタッフの方々も楽しそうに作業</span>
            してくれ、出し戻しの作業も和気あいあいとした雰囲気の中進めることができました。
          </p>
          <div className="PR__contentsBonenkaiImage">
            <button onClick={() => showMediaViewerForBonenkai(0)} className="PR__contentsBonenkaiImageThumbnail">
              <img src={thumbnailBonenkai} alt="幹事5,000人に忘年会大調査の画像" width="100%" height="auto" />
            </button>
        ...
        <section className="PR__contentsYoshimotozaka">
          <Heading level={2} text="吉本坂46の泣かせメシ！ - Yahoo!ライフマガジン" />
          <ul className="PR__contentsYoshimotozakaTags util-BadgeList">
            <li className="util-BadgeList__item">
              <Badge color="orange" text="タレント素材" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="短期開発" />
            </li>
            <li className="util-BadgeList__item">
              <Badge color="orange" text="全工程担当" />
            </li>
          ...
          <p>
            ...
            <br />
            レギュレーションは厳しくなかったものの、
            <span className="util-Text--highlighted">46人分の素材の編集に（「46」というなんともレイアウトの難しい数のおかげで）</span>
            苦労しました。
          </p>
          <div className="PR__contentsYoshimotozakaImage">
            <button onClick={() => showMediaViewerForYoshimotozaka(0)} className="PR__contentsYoshimotozakaImageThumbnail">
              <img src={thumbnailYoshimotozaka} alt="吉本坂46の泣かせメシ！　の画像" width="100%" height="auto" />
            </button>
        ...
        <section className="PR__contentsRolls">
          ...
          <ul className="PR__contentsRollsList">
            ...
            <li className="PR__contentsRollsListItem">
              <Icon type="hyphen" size="medium" color="primary" />
              <span>ビジュアルデザイン（アイコン作成 / イラストレーション / ウェブページデザイン）</span>
            </li>
            <li className="PR__contentsRollsListItem">
              <Icon type="hyphen" size="medium" color="primary" />
        ...
        <div className="PR__contentsBack">
          <Button href="./" color="orange" text="トップページへ戻る" iconLeft="arrow_left" isInline={true} target="_self" />
        </div>
      </div>

      {isVisibleMediaViewerForGhit && (
        <MediaViewer mediaList={mediaListForGhit} selectedIndex={selectedIndexForGhit} onClickHandlerForClosing={hideMediaViewerForGhit} />
      )}

      {isVisibleMediaViewerFor311 && (
        <MediaViewer mediaList={mediaListFor311} selectedIndex={selectedIndexFor311} onClickHandlerForClosing={hideMediaViewerFor311} />
      )}
